...
$queryci = "SELECT CI,NOMBRES from empleado where COD_EMPLEADO =".$_SESSION['user1'];
$m2ci= mysql_query($queryci) or die ("Por favor ingrese los datos correctamente2");
$nomproci= mysql_fetch_array($m2ci);
...
$query2 = "SELECT DESCRIPCION from proceso where COD_PROCESO =".$_SESSION['proin']." AND COD_MACROPROCESO=".$_SESSION['macroin'];
$m2= mysql_query($query2) or die ("Por favor ingrese los datos correctamente2");
$nompro= mysql_fetch_array($m2);

$query3 = "SELECT I.PORCENTAJE FROM indicador AS I INNER JOIN proceso AS PR ON I.COD_PROCESO=PR.COD_PROCESO WHERE I.COD_EMPLEADO=".$_SESSION['codin']." AND PR.COD_PROCESO=".$_SESSION['proin']." AND PR.COD_MACROPROCESO=".$_SESSION['macroin']." AND I.FECHA_INICIO='".$_SESSION['iiniin']."' AND I.FECHA_FIN='".$_SESSION['ffinin']."'";
$m3= mysql_query($query3) or die ("Por favor ingrese los datos correctamente3");
$porce= mysql_fetch_array($m3);

$cumple="CALL porcentajeIndicadoresxEscuela(".$_SESSION['proin'].",".$_SESSION['macroin'].",2,".$porce['PORCENTAJE'].")";
...
$cumplimiento=mysql_fetch_array($cump);
 ...
 $myPicture->drawText(110,200,"CI: ".$nomproci['CI']."           Nombre:  ".$nomproci['NOMBRES'],$TextSettings); 
 ...
 $myPicture->drawText(110,222,$escuelare[0],$TextSettings); 
   ...
   /* Write some text */  
 $TextSettings = array("R"=>255,"G"=>255,"B"=>255,"Angle"=>0,"FontSize"=>12); 
 $myPicture->drawText(110,244,"Fecha Inicio: ".$_SESSION['iiniin'],$TextSettings); 
 
   /* Write some text */  
 $TextSettings = array("R"=>255,"G"=>255,"B"=>255,"Angle"=>0,"FontSize"=>12); 
 $myPicture->drawText(110,266,"Fecha Fin: ".$_SESSION['ffinin'],$TextSettings); 
/* Write some text */  
