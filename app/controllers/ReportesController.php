...
class ReportesController extends BaseController {
          ...
          public function individual_bsc($e,$t)
    {
            ...
            if ($tiempo == 1) 
            {
                ...
                $macroprocesos=DB::select('SELECT distinct(m.COD_MACROPROCESO) as OBJETIVO,m.NOMBRE as DESCRIPCION 
                    ...
                    where m.COD_TIPO=3 and i.COD_EMPLEADO =? AND i.COD_ESCUELA=?',array($codigoEmpleado,$e));
        ...
        $tiempo=Login::tiempoSesion();
            ...
            if ($tiempo == 1) 
            {
                if (($tipo==1) || (($tipo==2 || $tipo==3) && $this->getEscuelaEmpleado()==1))
                {
                    ...
                    $ci=Auth::user()->COD_EMPLEADO;
                   // echo("<script>console.log('PHP: ".$tipoR."');</script>");
                    return View::make('reportes.mensual_empleado_bsc', array('escuela' =>$escuela,'tipoReporte'=>$tipoR));
                }   
                else
                {
                    return Redirect::back();
    ...
    //echo("<script>console.log('PHP: ".$tipoReporte."');</script>");
        ...
        public function buscarEmpleadoBalance()
    {
        $consulta=Input::get('consult');
        $escuela=Input::get('escuela');
        $tipoReporte=Input::get('tipoReporte');
       ...
       // echo("<script>console.log('PHP: ".$escuela."');</script>");
        ...
        return View::make('reportes.empleadosFotosBalance',array('empleados' => $empleados,'escuela'=>$escuela,'tipoReporte'=>$tipoReporte));
    }
        ...
        public function combo2_bsc()
    {
        ...
        if ($tipoReporte==1 || $tipoReporte==4) {
         ...
         if ($tipoReporte==2 || $tipoReporte==3) {
            return View::make('reportes.procesosBusqueda_bsc', array('procesos' => $proceso,'tipoEmpleado' => $tipoE,'macroproceso' => $macroproceso,'escuela' =>$escuela,'cedula'=>$cedula,'codigo'=>$codigo,'name'=>$name,'mail'=>$mail,'tipoReporte'=>$tipoReporte));
        }
        ...
        public function tabla_bsc()
    {
        ...
        if ($tipoReporte==1 || $tipoReporte==4) 
        {
            //echo("<script>console.log('PHP: ".$escuela."');</script>");
            $indicadores=DB::select('SELECT DISTINCT I.COD_INDICADOR,I.COD_PROCESO,I.FECHA_INICIO,I.FECHA_FIN,I.COD_MACROPROCESO,I.COD_EMPLEADO FROM indicador AS I INNER JOIN proceso AS P ON P.COD_PROCESO=I.COD_PROCESO WHERE I.COD_EMPLEADO=? AND I.COD_PROCESO=? AND I.COD_MACROPROCESO=? AND I.COD_ESCUELA=? ORDER BY I.FECHA_FIN DESC LIMIT 31',array($codigo,$proceso,$macroproceso,$escuela));
            return View::make('reportes.tabla_bsc', array('indicadores' => $indicadores,'escuela' =>$escuela,'macroproceso'=>$macroproceso,'proceso'=>$proceso,'codigoEmpleado'=>$codigo,'cedula'=>$cedula,'codigo'=>$codigo,'name'=>$name,'mail'=>$mail));
        }
        if ($tipoReporte==2 || $tipoReporte==3) 
        {
            $mes=Input::get('mes');
            ...
            return View::make('reportes.tablaMensual_bsc', array('escuela' =>$escuela,'macroproceso'=>$macroproceso,'proceso'=>$proceso,'codigoEmpleado'=>$codigo,'cedula'=>$cedula,'codigo'=>$codigo,'suma'=>$suma,'mes'=>$mes,'name'=>$name,'mail'=>$mail));
        }
    ...
    public function getMaximoBalance($macroproceso,$proceso)
    {
        $maximo=0;
        if ($macroproceso==20 && $proceso==1) { $maximo=1.5;}
        if ($macroproceso==20 && $proceso==2) { $maximo=4.5;}
        if ($macroproceso==20 && $proceso==3) { $maximo=3;}
        if ($macroproceso==20 && $proceso==4) { $maximo=1;}
        #p21
        if ($macroproceso==21 && $proceso==1) { $maximo=20;}
        #p22
        if ($macroproceso==22 && ($proceso==1 || $proceso==2 || $proceso==3)) { $maximo=1.33;}
        if ($macroproceso==22 && $proceso==4) { $maximo=4;}
        if ($macroproceso==22 && ($proceso==5 || $proceso==6 || $proceso==7 || $proceso==8)) { $maximo=2;}
        if ($macroproceso==22 && ($proceso==9 || $proceso==10 || $proceso==11 || $proceso==12)) { $maximo=1;}
        #p23
        if ($macroproceso==23 ) { $maximo=3.3;}
        #p24
        if ($macroproceso==24 ) { $maximo=4;}
        return $maximo;
    }
    ...
    {    
        ...
        //$cumplimiento=$this->getValorCumplido($proceso,$macroproceso,$escuela,$f1,$f2,$codigoEmpleado);
        #p20
        $maximo=$this->getMaximoBalance($macroproceso,$proceso);
    ...
    public function imagenReporteMensualBalance($escuela,$macroproceso,$proceso,$mes,$cedula,$codigo,$suma,$op)
    {    
        include("pChart2.1.4/class/pData.class.php");
        include("pChart2.1.4/class/pDraw.class.php");
        include("pChart2.1.4/class/pImage.class.php");
 ...
 public function imagenReporteConsolidadoFacultad()
    ...
    public function imagenReporteConsolidadoEscuela_bsc($escuela)
    {    
         $tiempo=Login::tiempoSesion();
        $tipo=Login::tipoEmpleado();
        $total=0;
    ...
    public function pdfReporte($escuela,$macroproceso,$proceso,$f1,$f2,$cedula,$codigo,$nombres,$mail)
    {
        ...
        $pdf->Output('images/'.$codigoEmpleado.'.pdf','D');
                ...
                public function pdfReporteBalance($escuela,$macroproceso,$proceso,$f1,$f2,$cedula,$codigo,$nombres,$mail)
    {
        $school=$this->getEscuela($escuela);
        $process=$this->getProceso($proceso,$macroproceso);
        $cedulaEmpleado=$cedula;
        ...
        $this->imagenReporteBalance($escuela,$macroproceso,$proceso,$f1,$f2,$cedulaEmpleado,$codigoEmpleado,2);
        ...
        $pdf->SetFont('ARIAL','B',15);
        $pdf->SetTextColor(12,41,68);
        $pdf->Cell(120, 0, utf8_decode("Reporte de Proyectos"), 0,"C", TRUE);
        $pdf->Ln(15);
        $pdf->Cell(20);
        $pdf->SetFont('ARIAL','',10);
        $pdf->SetTextColor(12,41,68);
        ...
        $pdf->Ln(8);
        //$pdf->Ln(9);
        $porc=round(100*$porcen/$maximo,2);
        //$porc=round($cumplimiento['cumple'],2);
        if($porc>=0 && $porc<=70)
        {
            $pdf->SetFillColor(255,0,0);
            $pdf->SetTextColor(255,255,255);
            $pdf->Cell(20);
            $pdf->Cell(150, 8, utf8_decode("PORCENTAJE:    ".$porcen*100/$maximo),"LRB", 0,"C", TRUE);
            //$pdf->Cell(150, 8, utf8_decode("PORCENTAJE:    ".$porc),"LRB", 0,"C", TRUE);
        }
         else {
            if($porc>70 && $porc<=90)
        ...
        public function pdfReporteMensual($escuela,$macroproceso,$proceso,$mes,$cedula,$codigo,$suma,$nombres,$mail)
    {
        ...
        //$pdf->Image("../pChart2.1.4/examples/pictures/".$_SESSION['user'].".png",20,120,180,45.5);
            ...
            public function pdfReporteMensualBalance($escuela,$macroproceso,$proceso,$mes,$cedula,$codigo,$suma,$nombres,$mail)
    {
        $school=$this->getEscuela($escuela);
        $process=$this->getProceso($proceso,$macroproceso);
        $cedulaEmpleado=$cedula;
        ...
        $pdf->SetFont('ARIAL','B',15);
        $pdf->SetTextColor(12,41,68);
        $pdf->Cell(135, 0, utf8_decode("Reporte Mensual de Proyectos"), 0,"C", TRUE);
        $pdf->Ln(15);
        $pdf->Cell(20);
        $pdf->SetFont('ARIAL','',10);
        $pdf->SetTextColor(12,41,68);
