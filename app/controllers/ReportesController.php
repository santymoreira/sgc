...
class ReportesController extends BaseController {
    ...
    {
        $Escuela = DB::select('SELECT NOMBRE FROM escuela where COD_ESCUELA=2');
         foreach ($Escuela as $esc) { $nombreEscuela=$esc->NOMBRE; }
        
        $Proceso = DB::select('SELECT DESCRIPCION from proceso where COD_PROCESO =1 AND COD_MACROPROCESO=1');
        foreach ($Proceso as $proc) { $nombreProceso=$proc->DESCRIPCION; }

        $Porcentaje = DB::select('SELECT DISTINCT(I.PORCENTAJE) FROM indicador AS I INNER JOIN proceso AS PR ON I.COD_PROCESO=PR.COD_PROCESO WHERE I.COD_EMPLEADO=73 AND PR.COD_PROCESO=1 AND PR.COD_MACROPROCESO=1 AND I.FECHA_INICIO=1 AND I.FECHA_FIN=1');
        if ($Porcentaje) {
